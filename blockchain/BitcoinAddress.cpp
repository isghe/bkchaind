							   ...
							   uint8_t output[25])		// A bitcoin address (in binary( is always 25 bytes long.
{
	...
	if (input[0] == 0x04 || input[0] == 0x02 || input[0] == 0x03)
	{
		uint8_t hash1[32]; // holds the intermediate SHA256 hash computations
		SHA256::computeSHA256(input, input[0] == 0x04 ? 65 : 33, hash1);	// Compute the SHA256 hash of the input public ECSDA signature
...
bool bitcoinPublicKeyToHash160(const uint8_t input[65], // The 65 bytes long ECDSA public key; first byte will always be 0x4 followed by two 32 byte components
	uint8_t output[20])		// Hash160
{
	bool ret = false;
	...
	if (input[0] == 0x04 || input[0] == 0x02 || input[0] == 0x03)
	{
		uint8_t hash1[32]; // holds the intermediate SHA256 hash computations
		SHA256::computeSHA256(input, input[0] == 0x04 ? 65 : 33, hash1);	// Compute the SHA256 hash of the input public ECSDA signature
		RIPEMD160::computeRIPEMD160(hash1, 32, output);	// Compute the RIPEMD160 (20 byte) hash of the SHA256 hash
		ret = true;
	}
	return ret;
}

bool bitcoinHash160ToAddress(uint8_t pubkeyAddress, const uint8_t input[20],
	uint8_t output[25])
{
	uint8_t hash1[32]; // holds the intermediate SHA256 hash computations
	output[0] = pubkeyAddress;	// Store a network byte of 0 (i.e. 'main' network)
	memcpy(&output[1], input, 20);
	SHA256::computeSHA256(output, 21, hash1);	// Compute the SHA256 hash of the RIPEMD16 hash + the one byte header (for a checksum)
	SHA256::computeSHA256(hash1, 32, hash1); // now compute the SHA256 hash of the previously computed SHA256 hash (for a checksum)
	output[21] = hash1[0];	// Store the checksum in the last 4 bytes of the public key hash
	output[22] = hash1[1];
...
bool bitcoinAddressToHash160(const uint8_t input[25],
	uint8_t output[20])
{
...
bool bitcoinHash160ToAscii(uint8_t pubkeyAddress, const uint8_t input[20], char *output, uint32_t maxOutputLen)
{
	bool ret = false;
	...
	if (bitcoinHash160ToAddress(pubkeyAddress, input, hash2))
	{
...
bool bitcoinAsciiToHash160(const char *input, uint8_t output[20]) // convert an ASCII bitcoin address into Hash160.
{
	bool ret = false;
	uint8_t address[25];
	uint32_t len = BASE58::decodeBase58(input, address, 25, true);
	if (len == 25) // the output must be *exactly* 25 bytes!
	{
		uint8_t checksum[32];
		SHA256::computeSHA256(address, 21, checksum);
		SHA256::computeSHA256(checksum, 32, checksum);
		if (address[21] == checksum[0] ||
			address[22] == checksum[1] ||
			address[23] == checksum[2] ||
			address[24] == checksum[3])
		{
			ret = true; // the cheksum matches!
			memcpy(output, &address[1], 20);
		}
	}
	return ret;
}
